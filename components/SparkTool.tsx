...
const SparkTool: React.FC = () => {
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const data = await getCreativeSpark(industry, vibe);
      setResult(data);
    } catch (err) {
      setError('Seed failed to sprout. Re-fertilizing...');
    } finally {
      setLoading(false);
    }
  };
          ...
          <div>
            ...
            <form onSubmit={handleGenerate} className="space-y-12">
               <div className="space-y-4">
                 <label className="text-[0.7rem] font-black uppercase tracking-[0.4em] text-ivy mb-2 block">Seed Sector</label>
                 <input 
                   type="text"
                   value={industry}
                   onChange={(e) => setIndustry(e.target.value)}
                   placeholder="e.g. Mycelium Architecture"
                   className="w-full bg-white border-2 border-ivy p-7 text-2xl font-bold outline-none focus:bg-wasabi/20 transition-all stamp-shadow placeholder:text-ivy/20"
                 />
               </div>
               ...
               <div className="flex flex-wrap gap-3">
                 {['Organic', 'Kinetic', 'Floral', 'Digital', 'Hybrid'].map((v) => (
                   <button
                     ...
                     type="button"
                     onClick={() => setVibe(v)}
                     className={`px-8 py-3 border-2 border-ivy font-black text-[0.7rem] uppercase tracking-widest transition-all ${vibe === v ? 'bg-ivy text-wasabi -translate-y-1' : 'bg-white text-ivy hover:bg-wasabi stamp-shadow'}`}
                   >
                     {v}
                   </button>
                 ))}
               ...
               <button 
                type="submit" 
                disabled={loading}
                className="w-full py-8 bg-ivy text-wasabi font-black uppercase tracking-[0.5em] text-xs hover:bg-coral hover:text-white transition-all stamp-shadow hover:translate-x-1 hover:translate-y-1 disabled:opacity-50"
              >
             ...
             <div className="bg-white p-12 md:p-16 border-2 border-ivy min-h-[650px] flex flex-col justify-between relative stamp-shadow overflow-hidden">
                ...
                {result ? (
                  <div className="relative z-10 animate-in fade-in zoom-in duration-700">
                    ...
                    <h3 className="font-serif text-6xl md:text-8xl font-black italic text-ivy leading-[0.85] mb-8 tracking-tighter">{result.brandName}</h3>
                    <p className="font-serif text-3xl text-coral font-bold italic mb-12">"{result.tagline}"</p>
                    ...
                    <div className="p-8 bg-cardstock border-2 border-ivy mb-12 italic relative overflow-hidden group">
                       ...
                       <p className="font-serif text-xl font-medium leading-relaxed text-ivy relative z-10">"{result.reasoning}"</p>
                    </div>
                    ...
                    <div className="flex items-center gap-12">
                       ...
                       <div>
                          ...
                          <p className="font-mono text-4xl text-ivy font-black tracking-tighter uppercase">{result.primaryColor}</p>
                       </div>
                    </div>
                  </div>
                ) : (
                  <div className="flex-1 flex flex-col items-center justify-center text-center opacity-10">
                    <div className="text-[12rem] mb-12 animate-float-slow">🌿</div>
